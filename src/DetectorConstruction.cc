...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    CLOVER_Distance[8] = 17*cm;
    CLOVER_phi[8] = 270*deg;
    CLOVER_theta[8] = 135*deg;
    ...
    {
        if(CLOVER_AllPresent_Override == true) CLOVER_Presence[i] = true;
        if(CLOVER_AllAbsent_Override == true) CLOVER_Presence[i] = false;
        if(CLOVER_AllPresent_Override == true && CLOVER_AllAbsent_Override== true) CLOVER_Presence[i] = false;
        
        if(CLOVER_Shield_AllPresent_Override == true) CLOVER_Shield_Presence[i] = true;
        if(CLOVER_Shield_AllAbsent_Override == true) CLOVER_Shield_Presence[i] = false;
        if(CLOVER_Shield_AllPresent_Override == true && CLOVER_Shield_AllAbsent_Override == true) CLOVER_Shield_Presence[i] = false;
    }
    ...
    */
    ...
    if(K600_BACTAR_sidesOn_Presence || K600_BACTAR_sidesOff_Presence || K600_BACTAR_beamRightSideOff_Presence || K600_BACTAR_beamLeftSideOff_Presence)
    {
        ...
        new G4PVPlacement(0,               // no rotation
                          ...
                          "BACTAR",       // its name
                          LogicWorld,//LogicVacuumChamber,         // its mother  volume LogicVacuumChamber
                          false,           // no boolean operations
                          0,               // copy number
                          fCheckOverlaps); // checking overlaps
    ...
    for(G4int i=0; i<numberOf_NAIS; i++)
    {
     ...
     /*   YperpW[i] = G4ThreeVector(std::cos(NAIS_theta[i]) * std::cos(NAIS_phi[i]), std::cos(NAIS_theta[i]) * std::sin(NAIS_phi[i]), -std::sin(NAIS_theta[i]));
        
        
        W[i]=G4ThreeVector(0.,0.,0.);
        U[i]=G4ThreeVector(0.,0.,0.);
        V[i]=G4ThreeVector(0.,0.,0.);
        
        
        W[i] = NAIS_position[i].Unit();
        U[i] = W[i].Cross(YperpW[i]);
        V[i] = W[i].Cross(U[i]);
        
        U[i] = U.Unit();
        V[i] = V.Unit();
        
        U[i].Rotate(beta_u * M_PI/180, U[i]);
        V[i].Rotate(beta_u * M_PI/180, U[i]);
        
        U[i].Rotate(beta_v * M_PI/180, V[i]);
        V[i].Rotate(beta_v * M_PI/180, V[i]);
        
        U[i].Rotate(beta_w * M_PI/180, W[i]);
        V[i].Rotate(beta_w * M_PI/180, W[i]);
        
        NAIS_transform[i] = (U[i],V[i],W[i]);
